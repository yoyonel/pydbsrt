...
from sqlalchemy import BIGINT, INT, TEXT, Column, ForeignKey, create_engine, text
...
def insert_img_hashes_from_media_into_db(media_path: Path) -> List[FrameItemPaired]:
    ...
    if session.query(TMedia.mediaHash).filter(TMedia.mediaHash == media_hash).scalar():
        logger.warning(
            "Media='%s' with hash='%s' already present in DB",
            str(media_path),
            media_hash,
        )
        return []
    ...
    frame_items = [
        TFrameItem(pHash=imghash_to_signed_int64(img_hash), frameOffset=offset_frame)
        for offset_frame, img_hash in enumerate(
            tqdm(ffmpeg_imghash_generator(str(media_path)))
        )
    ]
    ...
    session.commit()
    return [
        FrameItemPaired(i_frame_item, frame_item.dbId)
        for i_frame_item, frame_item in enumerate(frame_items)
    ]


def search_img_hash_in_db(
    search_phash: int, search_distance: int = 0
) -> List[FrameItem]:
...
WHERE "pHash" <@ ({search_phash}, {search_distance})"""
    ...
    with db.connect() as con:
        return [
            FrameItem(*tuple_frame_item) for tuple_frame_item in con.execute(statement)
        ]
...
def matching_with_db():
    ...
    params = dict(
        search_phash=imghash_to_signed_int64(img_hash_to_search), search_distance=0
    )
    with _Timer("Search random ImageHash (from media) into DB"):
        matched_frame_items = search_img_hash_in_db(**params)
    logger.info("count(search_img_hash(%s)=%d", params, len(matched_frame_items))
...
def insert_medias_into_db():
    ...
    for media_path in islice(root_media_path.glob("S02/*Homicide*.mkv"), 10):
        ...
        with _Timer("Insert ImageHashes (from media) into db"):
            insert_img_hashes_from_media_into_db(media_path)
