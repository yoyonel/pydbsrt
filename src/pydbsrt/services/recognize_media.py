...
def build_reader_frames(media: Path, nb_seconds_to_extract: float) -> Iterator[bytes]:
    ...
    ffmpeg_seek_input_cmd = ["-ss", str(datetime.timedelta(seconds=meta["duration"] // 2))]
    ffmpeg_seek_output_cmd = ["-frames:v", str(round(nb_seconds_to_extract * meta["fps"]))]
...
async def run(media: Path, search_distance: int, nb_seconds_to_extract: float) -> search_imghash_in_db.ResultRun:
    ...
    return await search_imghash_in_db.run(map(str, gen_signed_int64_hash), search_distance)
...
async def build_results(media: Path, results: search_imghash_in_db.ResultRun) -> List[BuildResult]:
    map_media_id_to_offsets_matched: Dict[int, List[PairedMatchedFrame]] = defaultdict(list)
    for record in results.records:
        for match in record.matches:
            map_media_id_to_offsets_matched[match.media_id].append(PairedMatchedFrame(record_search_offset=record.search_offset,
    ...
    async def _build_result(media_id, paired_matched_frames) -> BuildResult:
        ...
        return BuildResult(
            ...
            search_offsets_match={paired_matched_frame.record_search_offset for paired_matched_frame in paired_matched_frames},
            match_frames_offsets={paired_matched_frame.match_frame_offset for paired_matched_frame in paired_matched_frames},
            timer_in_seconds=results.timer.elapsed,
        )

    results = [await _build_result(media_id, paired_matched_frames)
...
def recognize_media(media: Path, search_distance: int, nb_seconds_to_extract: float):
    ...
    results_from_search_imghash_in_db: search_imghash_in_db.ResultRun = loop.run_until_complete(run(media, search_distance, nb_seconds_to_extract))
    results: List[BuildResult] = loop.run_until_complete(build_results(media, results_from_search_imghash_in_db))
