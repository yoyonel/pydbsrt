...
def export_imghash_from_media(media, output_file):
    ...
    gen_frame_hash = islice(gen_frame_hash, nb_frames_to_read + 0 * int(meta["fps"] * 60))
    chunk_nb_seconds = 15
    chunk_size = int(meta["fps"] * chunk_nb_seconds)
    console.print(f"Chunk size (nb frames): {chunk_size}")
    output_file = Path(output_file) if output_file else Path("/tmp/") / media.with_suffix(".phash").name
