...
async def search(term, num_results=10, lang="en", proxy=None):
    usr_agent = {
    ...
    async def fetch_results(search_term, number_results, language_code):
        ...
        if proxy:
            if proxy[:5] == "https":
                ...
                proxies = {"http": proxy}
        async with httpx.AsyncClient(proxies=proxies, headers=usr_agent) as client:
    ...
    html = await fetch_results(term, num_results, lang)
