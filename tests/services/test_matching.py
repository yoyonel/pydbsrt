...
async def test_search_phash_stream(conn, resource_phash_path):
    ...
    search_results = await search_phash_stream(
        map(str, (int64_first_phash,)), search_distance=0
    )
    search_record = search_results.records[0]
    # => 0 matching found
    assert (
        len(
            list(filter(lambda record: record.frame_offset == 0, search_record.matches))
        )
        == 0
    )
    # same research with search distance equal to 1 (fuzzy search)
    search_results = await search_phash_stream(
        map(str, (int64_first_phash,)), search_distance=1
    )
    search_record = search_results.records[0]
