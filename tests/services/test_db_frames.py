...
from services.db_frames import (
    import_binary_img_hash_to_db_async,
    agen_p_hash_from_media_in_db,
)
...
async def test_import_binary_img_hash_to_db_async(conn, resource_phash_path):
    ...
    expected_frames_inserted = len(
        list(gen_signed_int64_hash(binary_img_hash_file.open("rb")))
    )
    assert media_id == 1
...
async def test_agen_p_hash_from_media_in_db(conn, resource_phash_path):
    ...
    media_phashes_in_db_with_limit = [
        p_hash async for p_hash in agen_p_hash_from_media_in_db(media_hash, limit=32)
    ]
    ...
    media_all_phashes_in_db = [
        p_hash async for p_hash in agen_p_hash_from_media_in_db(media_hash)
    ]
